class OwnerOffersController < ApplicationController
  ...
  def create
    @owner_offer = BusinessOffer.new(owner_offer_params)
    @owner_offer.owner_id = params[:business_id]
    @owner_offer.offer_amount = owner_offer_params[:offer_amount].to_i
    @owner_offer.price_cents = (owner_offer_params[:offer_amount].to_f) * (1 - (business_offer_params[:discount].to_f / 100))
  ...
  def update
    @owner_offer = OwnerOffer.find(params[:id])
    authorize @owner_offer
    if @owner_offer.update(owner_offer_params)
      flash[:alert] = "Bravo!"
  ...
  def set_owner
    @owner = Owner.find(params[:business_id])
  end
